# Dicionário de Dados - Camada RAW (Bronze) - Formula 1 World Championship (1950-2024)
...
Códigos e descrições para o status final de um piloto no final de uma corrida (ex: terminou, acidente, falha mecânica).
...
| Campo | Tipo de Dado (Pandas) | Descrição |
|---|---|---|
| `statusId` | `int64` | **Chave primária.** Identificador único para o status. |
| `status` | `object` | Descrição do status final na corrida. **Ex:** 'Finished', 'Engine', 'Accident', 'Disqualified'. |
...
Informações geográficas e descritivas sobre os circuitos (pistas) de Fórmula 1.
...
| Campo | Tipo de Dado (Pandas) | Descrição |
|---|---|---|
| `circuitId` | `int64` | **Chave primária.** Identificador único do circuito. |
| `circuitRef` | `object` | Referência única de texto para o circuito. **Ex:** 'albert_park'. |
| `name` | `object` | Nome oficial do circuito. **Ex:** 'Albert Park Grand Prix Circuit'. |
| `location` | `object` | Cidade ou local onde o circuito está situado. |
| `country` | `object` | País do circuito. |
| `lat` | `float64` | Coordenada de latitude. |
| `lng` | `float64` | Coordenada de longitude. |
| `alt` | `int64` | Altitude do circuito em metros. |
| `url` | `object` | Link da Wikipedia para o circuito. |
...
Resultados de cada construtora (equipe) por corrida, focando em pontos e status geral.
...
| **Separador** | ',' |
| **Total de Colunas** | 5 |
| **Total de Registros** | 12625 |

### Campos do Dicionário

| Campo | Tipo de Dado (Pandas) | Descrição |
|---|---|---|
| `constructorResultId` | `int64` | **Chave primária.** Identificador único do resultado da construtora. |
| `raceId` | `int64` | **Chave estrangeira.** Identificador da corrida. |
| `constructorId` | `int64` | **Chave estrangeira.** Identificador da construtora (equipe). |
| `points` | `float64` | Pontos acumulados pela construtora naquela corrida. |
| `status` | `object` | Status da construtora na corrida. **Ex:** 'OK', 'Disqualified'. |
...
Resultados detalhados de pilotos e construtoras para as corridas **Sprint** (introduzidas em 2021).
...
| Campo | Tipo de Dado (Pandas) | Descrição |
|---|---|---|
| `resultId` | `int64` | **Chave primária.** Identificador único do resultado Sprint. |
| `raceId` | `int64` | **Chave estrangeira.** Identificador da corrida (evento). |
| `driverId` | `int64` | **Chave estrangeira.** Identificador do piloto. |
| `constructorId` | `int64` | **Chave estrangeira.** Identificador da construtora. |
| `number` | `int64` | Número do carro. |
| `grid` | `int64` | Posição no grid de largada para a corrida Sprint. |
| `position` | `object` | Posição final na corrida Sprint (pode ser texto). |
| `positionText` | `object` | Posição em texto. **Ex:** '1', 'R' (Retirado). |
| `positionOrder` | `int64` | Posição final ordenada na corrida Sprint. |
| `points` | `int64` | Pontos ganhos na corrida Sprint. |
| `laps` | `int64` | Número de voltas completadas. |
| `time` | `object` | Tempo final (para o vencedor) ou diferença (para os demais). |
| `milliseconds` | `int64` | Tempo ou diferença em milissegundos. |
| `fastestLap` | `int64` | Número da volta mais rápida do piloto na Sprint. |
| `fastestLapTime` | `object` | Tempo da volta mais rápida na Sprint. |
| `statusId` | `int64` | **Chave estrangeira.** Status final do piloto (relaciona-se com `status.csv`). |